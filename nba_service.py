...
class NBAStatsAPI:
    ...
    def get_player_stats(self, player_id):
        # just gets the most recent regular season - used for the initial page load
        try:
    ...
    def get_player_all_playoff_seasons(self, player_id):
        # same endpoint as regular season but reads SeasonTotalsPostSeason instead
        # only returns seasons where the player actually made the playoffs
        try:
            career = playercareerstats.PlayerCareerStats(player_id=player_id)
            data = career.get_normalized_dict()
            time.sleep(0.6)
 
            # if the player never made the playoffs this list will just be empty
            if not data['SeasonTotalsPostSeason']:
                return []
 
            # newest first, same as regular season
            return [
                self._build_season_dict(s)
                for s in reversed(data['SeasonTotalsPostSeason'])
            ]
